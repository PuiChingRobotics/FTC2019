...
abstract public class Nav extends LinearOpMode {
    ...
    public void turn_to_heading(double target_heading) {
        ...
        while (degrees_to_turn > .5 && opModeIsActive() && timeouttimer.seconds() < 30) {
            ...
            robot.Rfront.setPower(wheel_power);
            robot.Rback.setPower(wheel_power);
            robot.Lfront.setPower(-wheel_power);
            robot.Lback.setPower(-wheel_power);
    ...
    public void go_forward(double inches_to_travel, double heading, double speed, boolean gotocrater) {
        ...
        start_position_l_Front = robot.Lfront.getCurrentPosition();
        start_position_l_Rear = robot.Lback.getCurrentPosition();
        start_position_r_Front = robot.Rfront.getCurrentPosition();
        start_position_r_Rear = robot.Rback.getCurrentPosition();
        ...
        while (opModeIsActive() && !destination_reached && timeouttimer.seconds() < goforwardstopdetect && !craterhit) {
            ...
            robot.Lback.setPower(current_speed - power_adjustment);

            ticks_traveled_l_Front = Math.abs(robot.Lfront.getCurrentPosition() - start_position_l_Front);
            ticks_traveled_l_Rear = Math.abs(robot.Lback.getCurrentPosition() - start_position_l_Rear);
            ticks_traveled_r_Front = Math.abs(robot.Rfront.getCurrentPosition() - start_position_r_Front);
            ticks_traveled_r_Rear = Math.abs(robot.Rback.getCurrentPosition() - start_position_r_Rear);
    ...
    public double go_sideways(double angledegrees, double heading, double power, double inches) {
        ...
        while (opModeIsActive() && !mineralclose && !destinationreached) {
            ...
            ticks_traveled_l_Front = Math.abs(robot.Lfront.getCurrentPosition() - start_position_l_Front);
            ticks_traveled_l_Rear = Math.abs(robot.Lback.getCurrentPosition() - start_position_l_Rear);
            ticks_traveled_r_Front = Math.abs(robot.Lfront.getCurrentPosition() - start_position_r_Front);
            ticks_traveled_r_Rear = Math.abs(robot.Rback.getCurrentPosition() - start_position_r_Rear);
